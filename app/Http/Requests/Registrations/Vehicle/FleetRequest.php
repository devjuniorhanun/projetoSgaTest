...
namespace App\Http\Requests\Registrations\Vehicle;
...
// Exige que o campo seja informado.
            ...
            // Valida o campo conforme as regras de negócio.
// Exige que o campo seja informado.
            'fleet_brand_id' => ['required', 'integer', 'exists:fleet_brands,id'],
            // Valida o campo conforme as regras de negócio.
// Exige que o campo seja informado.
            'fleet_model_id' => ['required', 'integer', 'exists:fleet_models,id'],
            // Valida o campo conforme as regras de negócio.
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
            'name' => ['required', 'string', 'max:255', Rule::unique('fleets','name')->ignore($this->route('fleet')?->id ?? $this->route('fleet'))],
            // Valida o campo conforme as regras de negócio.
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
            'code' => ['required', 'string', 'max:255', Rule::unique('fleets','code')->ignore($this->route('fleet')?->id ?? $this->route('fleet'))],
            // Valida o campo conforme as regras de negócio.
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
            'plate' => ['required', 'string', 'max:255', Rule::unique('fleets','plate')->ignore($this->route('fleet')?->id ?? $this->route('fleet'))],
            // Valida o campo conforme as regras de negócio.
// Restringe o campo a um conjunto fechado de valores permitidos.
            'fleet_type' => ['sometimes', Rule::in(['P','T'])],
            // Valida o campo conforme as regras de negócio.
// Exige que o campo seja informado.
...
// Exige que o campo seja informado.
            ...
            // Valida o campo conforme as regras de negócio.
// Restringe o campo a um conjunto fechado de valores permitidos.
            'fuel_type' => ['sometimes', Rule::in(['A','B'])],
            // Valida o campo conforme as regras de negócio.
// Restringe o campo a um conjunto fechado de valores permitidos.
            'marking_type' => ['sometimes', Rule::in(['H','K'])],
            // Valida o campo conforme as regras de negócio.
// Exige que o campo seja informado.
            'starting_meter' => ['required', 'numeric'],
            // Valida o campo conforme as regras de negócio.
// Exige que o campo seja informado.
            'end_gauge' => ['required', 'numeric'],
            // Valida o campo conforme as regras de negócio.
// Restringe o campo a um conjunto fechado de valores permitidos.

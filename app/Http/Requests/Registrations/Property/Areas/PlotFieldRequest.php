...
namespace App\Http\Requests\Registrations\Property\Areas;
...
// Retorna o resultado da operação atual.
        $plotField = $this->route('plot_field');
        $plotFieldId = is_object($plotField) ? $plotField->id : $plotField;
        $fieldId = $this->input('field_id', is_object($plotField) ? $plotField->field_id : null);
        $cropId = $this->input('crop_id', is_object($plotField) ? $plotField->crop_id : null);
...
// Exige que o campo seja informado.
            ...
            'name' => [
                ...
                Rule::unique('plot_fields', 'name')->where(fn ($query) => $query
                    ->where('field_id', $fieldId)->where('crop_id', $cropId)->whereNull('deleted_at'))
                    ->ignore($plotFieldId),
            ],
            // Valida o campo conforme as regras de negócio.
// Exige que o campo seja informado.
...
// Exige que o campo seja informado.
            ...
            // Valida o campo conforme as regras de negócio.
// Exige que o campo seja informado.
            'variety_culture_id' => ['required', 'integer', 'exists:variety_cultures,id'],
            // Valida o campo conforme as regras de negócio.
// Valida o campo somente quando ele estiver presente na requisição.
            'area' => ['sometimes', 'numeric', 'gt:0'],
            // Valida o campo conforme as regras de negócio.
// Valida o campo somente quando ele estiver presente na requisição.
            'pms' => ['sometimes', 'numeric'],
            // Valida o campo conforme as regras de negócio.
// Valida o campo somente quando ele estiver presente na requisição.
            'linear_seed' => ['sometimes', 'numeric'],
            // Valida o campo conforme as regras de negócio.
// Valida o campo somente quando ele estiver presente na requisição.
            'start_planting' => ['sometimes', 'date'],
            // Valida o campo conforme as regras de negócio.
// Valida o campo somente quando ele estiver presente na requisição.
            'final_planting' => ['sometimes', 'date'],
            // Valida o campo conforme as regras de negócio.
// Valida o campo somente quando ele estiver presente na requisição.

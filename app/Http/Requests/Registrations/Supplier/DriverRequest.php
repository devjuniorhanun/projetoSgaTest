...
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
            'name' => ['required', 'string', 'max:255', Rule::unique('drivers','name')->ignore($this->route('driver')?->id ?? $this->route('driver'))],
            // Valida o campo conforme as regras de negócio.
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
            'code' => ['required', 'string', 'max:255', Rule::unique('drivers','code')->ignore($this->route('driver')?->id ?? $this->route('driver'))],
            // Valida o campo conforme as regras de negócio.
// Valida que o valor não esteja duplicado no banco, ignorando o registro atual na edição.
            'plate' => ['required', 'string', 'max:255', Rule::unique('drivers','plate')->ignore($this->route('driver')?->id ?? $this->route('driver'))],
            // Valida o campo conforme as regras de negócio.
